...
function apiLocationBased(lat, lng, radius, contentTypeId = "") {
  ...
  return $.ajax({
    url: `${PROXY_BASE}/api/tour/location?${qs}`,
    type: "GET",
    dataType: "json",
  })
    .then(parseTourItems)
...
function apiKeyword(keyword, contentTypeId = "") {
  ...
  return $.ajax({
    url: `${PROXY_BASE}/api/tour/keyword?${qs}`,
    type: "GET",
    dataType: "json",
  })
    .then(parseTourItems)
...
function parseTourItems(data) {
  console.group("📡 TourAPI Response Check");
  try {
...
async function fetchWeatherByCoords(lat, lng) {
  try {
    return await $.ajax({
      url: `${PROXY_BASE}/api/weather/coords?lat=${lat}&lon=${lng}`,
      type: "GET",
      dataType: "json",
    });
  } catch (e) {
...
async function fetchWeatherByCity(city) {
  try {
    return await $.ajax({
      url: `${PROXY_BASE}/api/weather/city?q=${encodeURIComponent(city)}`,
      type: "GET",
      dataType: "json",
    });
  } catch (e) {
...
async function fetchForecastByCoords(lat, lng) {
  try {
    return await $.ajax({
      url: `${PROXY_BASE}/api/weather/forecast?lat=${lat}&lon=${lng}`,
      type: "GET",
      dataType: "json",
    });
  } catch (e) {
  ...
  document.getElementById("dbg-test").addEventListener("click", async () => {
    ...
    try {
      const text = await $.ajax({
        url: url,
        type: "GET",
        dataType: "text", // 원본 텍스트를 받기 위함
      });

      log(`HTTP 상태: 200 OK`, "#5ce89a");
      ...
      log(`HTTP 상태: ${xhr.status} ${xhr.statusText}`, "#ff6b6b");
      log("❌ ajax 실패: " + xhr.statusText, "#ff6b6b");
...
// /**
//  * SKYWATCHER  — app.js
//  * OpenWeatherMap + 한국관광공사 TourAPI - fetch ver
//  */
